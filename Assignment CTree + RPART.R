...
ctree_classification<-rep("1",4799)
ctree_classification[ctree_probabilities[,2]<0.22108]="0" #Predict classification using 0.6073 threshold. Why 0.6073 - that's the average probability of being retained in the data. An alternative code: logistic_classification <- as.integer(logistic_probabilities > mean(testing$Retained.in.2012. == "1"))
...
prunned_rpart_tree<-prune(rpart_tree, cp=0.00086) #Prun the tree. Play with cp to see how the resultant tree changes
